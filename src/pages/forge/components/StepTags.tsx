...
export default function StepTags({ onNext }: Props) {
  ...
  const handleNext = () => {
    ...
    const answerValues = Object.values(answers).filter((value) => value?.trim());
    const tags = Array.from(new Set(answerValues)).slice(0, 6);
    const previousProfile = (() => {
      try {
        return JSON.parse(localStorage.getItem('star_profile') || '{}');
      } catch {
        return {};
      }
    })();
    ...
    localStorage.setItem(
      'star_profile',
      JSON.stringify({
        nickname: previousProfile.nickname || '夜航者',
        avatar: previousProfile.avatar || 'constellation',
        tags: tags.length > 0 ? tags : ['深夜独处', '被看见', '慢热连接'],
        answers,
      }),
    );

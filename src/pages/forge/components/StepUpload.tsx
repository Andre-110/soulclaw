...
export default function StepUpload({ onNext }: Props) {
  ...
  const handleAnalyze = () => {
    if (completedCount === 0 && boundCount === 0) return;
    setAnalyzing(true);
    setTimeout(() => {
      setAnalyzing(false);
      onNext();
    ...
    <div className="flex flex-col gap-4">
      <div className="rounded-2xl p-5 text-center" style={GLASS}>
        ...
        <p className="text-xs font-noto" style={{ color: 'rgba(224,239,255,0.55)' }}>
          粘贴主页链接或上传截图，同时会自动读取你在设置页已绑定的平台
        </p>
      </div>
        ...
        {PLATFORMS.map((p) => {
          ...
          return (
            <div key={p.id} className="rounded-2xl overflow-hidden" style={GLASS}>
              ...
              >
                <div className="w-9 h-9 flex items-center justify-center rounded-xl flex-shrink-0" style={{ background: `${p.color}22` }}>
                ...
                {isDone ? (
                  <div className="flex items-center gap-1 px-2.5 py-1 rounded-full" style={{ background: 'rgba(0,209,255,0.12)', border: '1px solid rgba(0,209,255,0.35)' }}>
                  ...
                  <i className={isOpen ? 'ri-arrow-up-s-line text-sm' : 'ri-arrow-down-s-line text-sm'} style={{ color: 'rgba(224,239,255,0.4)' }} />
                )}
              </button>

              {isOpen && (
                <div className="px-4 pb-4 flex flex-col gap-3" style={{ borderTop: '1px solid rgba(255,255,255,0.08)' }}>
      ...
      {completedCount > 0 && (
        <div className="flex items-center gap-2 px-4 py-3 rounded-xl" style={{ background: 'rgba(0,209,255,0.08)', border: '1px solid rgba(0,209,255,0.25)' }}>
      ...
      {boundCount > 0 && (
        <div className="flex items-center gap-2 px-4 py-3 rounded-xl" style={{ background: 'rgba(108,92,231,0.08)', border: '1px solid rgba(108,92,231,0.22)' }}>
          <i className="ri-links-line" style={{ color: '#A29BFE' }} />
          <span className="text-xs font-noto" style={{ color: '#C3B5FF' }}>
            检测到设置页已绑定 {boundCount} 个平台，生成报告时会一并抓取
          </span>
        </div>
      )}
      ...
      <button
        ...
        disabled={completedCount === 0 && boundCount === 0}
        ...
        style={{
          background: completedCount > 0 || boundCount > 0 ? 'linear-gradient(135deg, #6C5CE7, #00D1FF)' : 'rgba(255,255,255,0.06)',
          color: completedCount > 0 || boundCount > 0 ? '#fff' : 'rgba(224,239,255,0.3)',
          boxShadow: completedCount > 0 || boundCount > 0 ? '0 0 24px rgba(108,92,231,0.5)' : 'none',
        }}
      >
        {analyzing ? (
      ...
      <button onClick={onNext} className="text-center text-xs font-noto cursor-pointer py-1 whitespace-nowrap" style={{ color: 'rgba(224,239,255,0.4)' }}>
        跳过，直接回答问题 &rarr;
      </button>
    </div>
  );
